...
for k in [20,25,30,35,40,45,50]:
	...
	for run_id in range(5):
		...
		'''	
		book = np.ones((K,no_columns ))

		t1 = time.time()
		for i in range(2,K+1):
			for j in range(1,no_columns+1):
				book[i-1,j-1] = int( 1*( math.ceil(j*1.0/(2**(K-i)))%2 == 0) )
	
		print("time taken to create book:", time.time()-t1)
		#exit()
		#print(book,book.shape)

		book[ book == 0 ] = -1

		feas_col_indices = random.sample(range(0, no_columns), 2**12) 
		feas_col_indices.sort()
		#print(feas_col_indices)	

		book_feas = book[:,feas_col_indices]
		book_feas = book_feas.astype('int8')

		#print(book_feas)
		print("no of feas cols:",book_feas.shape[1])
		'''


		no_columns_feas = 2000#2**14
		feas_col_indices = sample_col_indices_sorted(0, no_columns-1, no_columns_feas) 
		print("feas_col_indices found ...")
		print("max feas_col_indices:", max(feas_col_indices) , math.log(max(feas_col_indices) ,2)  )
		#continue	

		book_feas = np.ones((K,no_columns_feas ))
		
		for i in range(2,K+1):
			for k_j,j in enumerate(feas_col_indices):
				book_feas[i-1,k_j] = int( 1*( math.ceil((j+1)*1.0/(2**(K-i)))%2 == 0) )
	
		print("book initialised...0,1...")
		book_feas[ book_feas == 0 ] = -1
		book_feas = book_feas.astype('int8')
		print("book initialised...-1+1....")
		#print(book_feas)
	
		#d_h_row = 0.5*(book_feas.shape[1] - np.matmul(book_feas,book_feas.T))
		...
		#m.Params.CoverCuts = 2
		#m.Params.ZeroHalfCuts = 2
		m.Params.TimeLimit = 1800#(7200+7200)
		m.Params.method = 0
		#m.Params.Heuristics = 0.5
		m.optimize()
		print("Runtime:", m.Runtime)
